...
/**
 * tracker处理用到的锁, 因为被static限制，所以只在本
 * 文件内用到：
 * 1. 对accept进行加锁
 * 2. 对g_tracker_thread_count进行加锁
 **/
static pthread_mutex_t tracker_thread_lock;
/*负载均衡用到的锁*/
static pthread_mutex_t lb_thread_lock;
/**
 * 记录tracker_thread的数目,用于等待线程被关闭，
 * 当该值为0时，表示所有开启的线程已经关闭，可
 * 继续做其他收尾工作了
 **/
...
/*
 * storage server list
 * 当前连接的组的所有storage的brief简要信息发送回去
 */
        ...
        const int status)
{
    ...
    /*当前连接的组的所有storage的brief简要信息写到缓冲*/
...
/**
 * 检查发起请求的Storage及其所在group是否脏，脏则引用数减一,
 * 并添加当前发起请求的Storage及其group到tracker维护的数据中
 **/
        ...
        const int64_t nInPackLen)
{
    ...
    do
    {
        ...
        /*接收到server_count个FDFSStorageBrief结构体*/
        ...
        const int64_t nInPackLen)
{
    ...
    do
    {
        ...
        *(body.domain_name + (sizeof(body.domain_name) - 1)) = '\0';
        ...
        /*解析完数据包后，把该Storage添加到TrackerGroups中*/
        status = tracker_mem_add_group_and_storage(pClientInfo, \
                &joinBody, true);
    } while (0);

    /*响应Join,把执行状态，需要同步的机器的IP发回去*/
        ...
        TrackerClientInfo *pClientInfo, const int64_t nInPackLen)
{
    ...
    do
    {
        ...
        *(pNewIpAddr + (IP_ADDRESS_SIZE - 1)) = '\0';
        ...
        /**
         * ip变了，该storage要清空相关数据，从新来了，然后还需要
         * 记录到changelog
         **/
...
static int tracker_check_logined(TrackerClientInfo *pClientInfo)
{
    ...
    /*
     * 如果pClientInfo->pGroup或pClientInfo->pStorage为空,
     * 表示还没登陆（join的时候是登陆），返回EACESS
     */
    memset(&resp, 0, sizeof(resp));
    resp.cmd = TRACKER_PROTO_CMD_STORAGE_RESP;
    resp.status = EACCES;
...
/*根据发过来的组名和storage的ip找对应的源头storage*/
...
/*处理请求同步目标storage的内容到发起了请求的节点的查询*/
/*与前一个函数的区别是不需要查找了*/
...
void* tracker_thread_entrance(void* arg)
{
    ...
    while (g_continue_flag)
    {
        ...
        /*因为多条线程在accept,为了避免惊群，需要对accept上锁*/
